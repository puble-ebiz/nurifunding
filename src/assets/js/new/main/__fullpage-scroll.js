...
setFullpage = function(moveBoxName, selectBoxName, moveSpeed, lastBoxIs, controlBoxName, handerName, fromSize = 720){
    ...
    this.setEventList = function(eventList){
        fullPageInfo.eventList = eventList;
        
        eventListAction(fullPageInfo);
    }
...
let eventListAction = function(info){
    // console.log("이벤트 동작");
    // console.log(selectBox.eq(info.index))
    $.each(info.eventList, function(num, item){
        // console.log(item.sectName);
        // console.log(item.actionOn);
        if(item.oneActionOn != true){
            if(info.selectBox.eq(info.index).hasClass(item.sectClassName)){
                item.eventFunc();
